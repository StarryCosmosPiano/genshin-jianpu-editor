...
const FEATURE_TOPICS: Topic[] = [
  {
    title: "打开与保存文件",
    body: [
      "左上角 **创建** 可新建 JPW 简谱、键盘谱或数字谱。工具栏 **打开** 支持 `.jpwabc`、键盘谱/数字谱 `.txt`、`.mid` / `.midi`、`.xml` / `.musicxml`、`.abc`；也可把这些文件**直接拖进**右侧谱面区。",
  ...
  },
  {
    title: "导出",
    body: [
      "工具栏 **导出**。简谱模式可导出 **PNG**（当前页）、**PPTX**（矢量，逐页成幻灯片）、**MIDI**，也可导出 **键盘谱 TXT**、**数字谱 TXT** 和 **JPW 简谱**。TXT 导出会询问保留多声部隐形标记，还是合并为不带标记的单声部。",
  ...
  },
  {
    title: "选项",
    body: [
      "工具栏 **选项** 可设置：谱面比例（16:9 / 4:3 / A4 / A3）、页面方向（横向 / 纵向）、钢琴乐器名称、每页行数、字号（基础 / 标题 / 词曲信息）、颜色；新安装默认使用 A4 纵向。",
      "多声部简谱还有各声部音量；混排模式下可勾选「隐藏小节号」。",
      "工具栏 **排版** 提供实时样张，可调整数字大小与粗细、真实音符横距、每行目标小节数、时值间距强度、末行铺满、谱行上下间距、和弦间距、八度点大小、八度点贴音距离、八度点与相邻和弦音的留白、升降号大小及其与数字的净空、延音线续音灰显、钢琴花括号与小节线。八度点会贴近自己的数字，并在下一行数字前保留独立空白。短时值音符会比长时值音符更紧；普通小节优先等宽，密集小节自动借用同行其他小节的空间，仍放不下时再推到下一行。谱行上下间距会参与自动分页：调小会把下一页能容纳的谱行回填上一页，调大到放不下时才换页。跨越多个相同音或和弦的延音线会逐段连接，第二段以后的续音均灰显；播放和 MIDI 导出只产生一次持续发音。钢琴谱每行小节号与花括号左边缘对齐；三连音按 3:2 均分时值。钢琴专用项已单独标注；点击「应用到整个软件」会保存并立即重排当前谱面，取消则恢复原样。",
      "**节奏刻度线** 默认开启。长刻度始终对应拍号中的每一拍；默认自动模式逐小节读取实际最短时值并补短刻度，也可切换手动模式，固定指定 4 / 8 / 16 / 32 / 64 分音符作为最短网格。所有刻度都按量化时刻与音符中心对齐，并在数字下方独占一行；钢琴双行谱在左手下方显示双手共享刻度线。",
    ],
  },
  {
    title: "键盘快捷键",
