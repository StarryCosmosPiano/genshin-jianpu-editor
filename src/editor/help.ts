...
const FEATURE_TOPICS: Topic[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "长篇文本录入时可点击顶部 **锁定谱面** 暂停实时重排；再次点击“解锁谱面”会一次更新到最新文本。文本区与谱面之间的窄箭头可隐藏/展开编辑器，**选项 → 文本编辑器位置** 可把文本区切换到页面右侧。",
  ...
  },
  {
    ...
    body: [
      ...
      "工具栏 **排版** 把数字、页面、刻度线、标题、钢琴系统和小节线分别放在可折叠菜单中，右侧完整样张在滚动设置时始终保持可见。可调整数字大小与粗细、真实音符横距、每行目标小节数、时值间距强度、末行铺满、谱行上下间距、和弦间距、八度点、升降号、延音线续音灰显、钢琴花括号与小节线。页面与谱行区域还可分别调整标题、副标题、调号/拍号/速度、作词/作曲/编曲的字号及横纵位置，并控制调拍速度信息到第一谱行的距离。拖动控件只改变对话框样张；点击「应用到整个软件」才会保存并重排当前及以后打开的谱面，点击「取消」直接放弃，改动后误点窗口外会先询问是否应用。",
