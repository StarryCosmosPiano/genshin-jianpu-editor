...
import { getShortcutBindings, SHORTCUT_ACTIONS, shortcutLabel } from "./shortcuts";
...
const FEATURE_TOPICS: Topic[] = [
  {
    title: "打开与保存文件",
    body: [
      "顶部 **文件 → 新建乐谱** 可创建 JPW 简谱、键盘谱或数字谱；**文件 → 打开文件 / 导入乐谱** 支持 `.jpwabc`、键盘谱/数字谱 `.txt`、`.mid` / `.midi`、`.xml` / `.musicxml`、`.abc`。也可把文件拖到编辑器任意区域导入；图片和 PDF 会进入本地识谱。",
  ...
  },
  {
    title: "编辑与实时排版",
    body: [
      "启动默认显示键盘谱示例，左侧文本谱展开。点谱面边缘箭头可收起或展开，拖动分隔条调整宽度。编辑原文后停顿约 0.2 秒，谱面自动更新。启动行为可在设置中修改。",
      "长篇文本录入时可点击文本区顶部 **锁定谱面** 暂停实时重排，亮金色表示已锁定；再次点击会一次更新到最新文本。**设置 → 文本编辑器位置** 可把文本区切换到页面右侧。谱面选中音符、打谱箭头和光标使用浅紫色 #A98DD4；文本编辑区保持实底金色配深色文字，文本插入光标也保持金色。",
      ...
      "主音和左上角的倚音都可直接选中：谱面与源码会双向高亮。普通选择模式下，`1–7`、`←/→`和 `Ctrl/Cmd+←/→` 不会改写乐谱，避免误触；需要改音高、移动起音或调整时值时请先进入 **打谱**。Del / 退格仍可把选中的音符或速度标记软删除为半透明，双击可恢复，Ctrl/Cmd+Z 可撤回。",
  ...
  },
  {
    title: "打谱输入模式",
    body: [
      "点击工具栏 **打谱**，或选中音符后按 `N` 进入打谱；点击 **选择** 或按 `Esc` 退出。未开启“谱面显示键盘按键”时，打谱中再按 `N` 也可退出；开启后 `N` 是低音琴键，不会退出。没有选中音符时先点击谱面确定起点，状态栏会提示等待选择；不会擅自从开头输入。浅紫色节奏光标吸附真实刻度，和弦上方或下方的新增位置显示虚线占位框。",
      "键盘谱开启 **谱面显示键盘按键** 后，选中音符可按 `Z–M`、`A–J`、`Q–U` 直接改为低、中、高三排对应音，保留时值及和弦里的其他音。普通选择中的 `N` 优先进入打谱，进入后按 `N` 才改音；文本编辑框中的字母仍正常输入。",
      "可点击小节内从数字上方到节奏刻度线之间的任意位置；点在两个刻度之间时会吸附到最近刻度。← / → 按当前自动或手动刻度移动光标，输入 `1–7` 后不会自动前进。光标落在休止处时按当前刻度建立音符；落在已有音上时直接改该音的唱名，并保留其时值和八度层级。JPW、键盘谱和数字谱中的黑色起音与灰色延音都属于同一逻辑音：在任意一段上改数字或按 `Ctrl/Cmd+↑/↓` 改八度，整条延音链会同步变更且不断线。按 `0` 会在当前刻度插入休止，`Delete` / `Backspace` 会把选中的音或整列改成休止。",
      "已有和弦处按 ↑ / ↓ 会逐个选中同拍的每个音；越过最高音或最低音后各有一个虚线新增位置，再继续移动才切换到相邻谱行。随后输入数字即可修改当前音，或在虚线位置加入继承原和弦时值的新音。以中音 3 为参照，上方输入 1/2/3 自动成为高八度，下方输入 3–7 自动成为低八度。`Ctrl/⌘+↑/↓` 改八度，`Ctrl/⌘+←/→` 缩短或延长当前音，`Alt+←/→` 按刻度移动当前音，`Alt+↑/↓` 把音移到相邻声部或乐器轨道；Alt 跨声部在普通选择模式也同样可用。",
      "进入打谱不会额外追加独立空小节；只有实际输入或延长的音越过现有结尾时，才会按当前拍号创建所需小节。离开打谱时会按“保留 0 休止符”设置完成各声部：开启时保留并补齐 0，关闭时删除可由前音覆盖的内部 0，让前音延续到下一起音或小节线；没有前音的开头静默仍保留休止。",
      "点选输入位置会立即试听当前音；按住 Ctrl/Cmd 点击同拍位置时会同时试听该和弦及其他对齐声部。触屏设备进入打谱后会显示 0 休止、1–7 和上下和弦按钮。播放整首时若打谱光标位于实际音符，会从该位置开始。",
      "在打谱模式下右键不会改变光标位置，菜单始终作用于当前光标或选区。选择“三连音”会从光标起把**两个当前写入时值**变成三个等分的 3:2 格（例如 16 分时值创建一个八分音符范围的三连音）；已有的前两个起音会吸附进去，空位自动补 0，光标前后延续的音会在边界拆成延音，容器不能跨小节。进入三连音后，输入、Alt 移动和 Ctrl/Cmd 调整时值都固定采用该三连音成员的时值，不受顶部全局时值影响，也不能把成员移动或延长到容器之外。多个声部的三连音重叠时只绘制覆盖最长范围的一个括号，各声部仍按元数据中的独立起点和成员时值推进；同一范围内的普通声部音符只参与合并排版，不会被变成三连音。三连音数字/括号本身可以点选，按 Del/退格即可解除；成员会回到创建前的二进制刻度，附点值向上归并，空间不足时只保留能完整放下的前置成员。菜单还可添加琶音、跨谱行琶音、上/下波音、Tr、倚音、延音/连奏、延长号、渐快/渐慢、速度、自由文本和反复记号；也可用五度圈更换调号，或从当前小节更换拍号。波音与 Tr 只用于单音，变成和弦会自动移除；键盘谱/数字谱还会检查花括号、方括号是否已分配相应功能。",
    ],
  },
  {
    title: "翻页与缩放",
  ...
  },
  {
    ...
    body: [
      ...
      "识别完成后，**文件 → 识图** 可切换到「二值图 + 半透明识别结果叠加」的核对视图，视图选择提供 附近浮窗 / 原位叠加 / 仅原图 三种方式；点识别对象可定位到对应文本。",
  ...
  },
  {
    ...
    body: [
      ...
      "打开或拖入 MusicXML 后会先显示导入设置，可选择 JPW 简谱、键盘谱、数字谱或保留五线谱混排，并可修改文本谱最短时值、乐器声部、标题署名、调号、拍号和速度。MusicXML 自带的音高与时值不会被重新量化。文件损坏、无有效音符或转换失败时会显示“导入失败”，且不会覆盖当前乐谱。",
    ],
  },
  {
    title: "导入 MIDI 与智能量化",
    body: [
      ...
      "输出键盘谱或数字谱时不会照搬 MIDI Note Off 或踏板形成的延音尾部，而是按文本谱的默认规则让每个起音延续到下一起音，再写出相邻时值符号。`{}`、`[]`、`|| ||`、`<>`、`()` 五组括号都可分配为和弦、倚音、琶音或三连音；默认 `{}` 为琶音、`[]` 为三连音、`<>` 为倚音、`()` 为和弦。未设置音符自身时值时，紧贴主音之前的纯音符会成为不占拍长的倚音，例如 `AS` 和 `A(BS)`；启用音符自身时值后，倚音必须使用已分配的括号。修改映射时可选择把原文括号按原功能同步迁移；留空但原文仍有该括号会标红提示。",
      "文本谱输出还可选择 **保留 0 休止符**：开启时保留 MIDI/MusicXML 中的显式休止，关闭时内部空白由前一个起音延长填充；全曲或某声部开头没有前音可延续的静默仍会写成 0。此选择会保存到 `@jpeditor` 元数据并可在乐谱设置中修改。",
      "琶音候选必须在同轨同通道内包含至少三个严格上行音，落在同一量化格、起音间隔很短且多数音彼此重叠，因此普通快速音阶不会被误折叠。默认 `<A>{BC}` 表示 A 是不占小节拍长的前置倚音、BC 是琶音；只有一个声部滚奏时写成 `(B{DG})`，跨越多个声部的同一条琶音写成 `{BDG}`。括号内外会组成一个和弦，共用时值、延音和选择高亮，波浪线只覆盖括号内音。未分配到括号的检测类型仍保留为普通音符。成品谱会把倚音放在主音左上角并画两条十六分减时线，再用一条两端留白的小弧线指向主音；纵向和弦的琶音波浪线会按最高、最低音自动伸缩。",
  ...
  },
  {
    title: "键盘谱 / 数字斜杠谱 TXT",
    body: [
      "键盘谱使用 `A–Z` 键位，数字谱使用 `1–7`，`+` / `-` 表示高低八度；`(AB)` 或 `(-47)` 表示同时按下的纵向和弦。默认是单谱行，也可在设置中启用 2–9 个同步声部。",
      "导入或乐谱设置中可开启 **谱面显示键盘按键**：右侧把数字替换成对应的 A–Z 键位，超出 Z–U 三排的音用上下加点表示额外八度。开启后还可选择把延音续音显示成 `0`，或完全隐藏续音字母（同时选择时隐藏优先）；这些都只是显示方式，原始 TXT、延音、播放、选音与导出音高不会改变。",
      ...
      "导入或创建键盘谱/数字谱后，可随时点击工具栏 **乐谱** 打开右侧设置，修改谱型识别、声部数量、乐器名称、速度、调号、拍号、时值符号、五种括号功能和标题署名；应用后会立即重解析当前文本并更新谱面与播放。",
      "乐谱设置中的 **保留 0 休止符** 默认开启。关闭后，已完成小节内没有新起音的空白会写成前音的延续时值；打谱时尚未离开的当前小节仍显示 0，便于看清可输入位置。",
      "多声部使用音符前的 `U+2063 INVISIBLE SEPARATOR` 数量编码：一个为 V1、两个为 V2，以此类推；没有标记的音属于最后一个默认声部 VN。`Alt+1`–`Alt+9` 可给文本或谱面中选中的单音/和弦音分配声部；再次按同一非默认声部快捷键会移回默认声部。单声部第一次按 `Alt+1` 会自动建立上下双行谱。打谱光标创建的三连音会在元数据中记录所属声部，容器里的 `0` 同样保留该声部标记；直接手写且没有元数据的三连音按公共多声部时间轴解释。",
      "编辑器会把这些不可见声部标记视为后方音符的一部分：左右方向键一次跨过整段标记，删除音符时标记一并删除，不会留下孤立的不可见字符。",
      ...
      "只要一个拍组含 `-` 且没有其他实际音符，就表示整组休止；因此 `/ - /`、`-/-/`、减号左右带空格或时值字符的结果相同，而数字谱的 `-1` 仍是低八度音。括号可重新分配为和弦、倚音、琶音或三连音；未设置音符自身时值时，连续贴写的 `ABC` 表示 A、B 是 C 的倚音，不再创建低于当前最小时值的普通音符。`A.B.C.` 仍分别使用点号映射的时值。",
  ...
  },
  {
    ...
    body: [
      ...
      "钢琴谱首页直接显示标题区：标题 / 副标题居中，调号 / 拍号 / BPM 在左，作词 / 作曲 / 编曲在右；调号、拍号和速度使用加大的成品谱字号，右下页码使用较小字号。每个系统以花括号加左竖线开始、双实线结束。花括号左侧只在全谱第一个系统显示较小的乐器名，默认是“钢琴”，可在排版中修改或在 `.Title` 写 `Instrument = {中文乐器名}`；后续系统不重复乐器名，但花括号最左尖点会与首个乐器名左边缘对齐。",
      "播放时双手同时发声并同步高亮；设置中可分别调整右手、左手音量，MIDI 导出为两个独立轨道。",
    ],
  },
  {
    title: "MIDI 多轨总谱",
    body: [
      ...
      "“排版”中的纸张尺寸可选择 A3 横向或纵向；播放混音和再次导出 MIDI 会按总谱声部分轨处理，并写入乐器/声部轨道名称。",
    ],
  },
  {
    title: "混排与乐句排版",
  ...
  },
  {
    title: "播放",
    body: [
      "工具栏点击 **播放** 开始试听，同一按钮变为 **停止**，再次点击即可停止；播放时当前音符会高亮。普通选择模式下选中音符后按 **空格**，从当前主选择处继续播放后面的乐谱，再按停止。长按不会反复启停。打谱模式的空格仍按书写时值前进，文本输入框也保留空格输入。点击谱面空白处清除选择后，工具栏播放会从全曲开头开始。",
      "多声部时可在 **设置** 里调各声部音量。",
  ...
  },
  {
    title: "导出",
    body: [
      "工具栏 **导出**。简谱模式可导出全部页面的 **PNG** 或 **PDF**、**PPTX**（矢量，逐页成幻灯片）、**MIDI** 和 **MusicXML**，也可导出 **键盘谱 TXT**、**数字谱 TXT** 和 **JPW 简谱**。MusicXML 会保留当前多声部、和弦、休止、拍号、调号、速度、延音/连音与歌词，便于继续在其他制谱软件中编辑。PNG 默认使用透明背景；多页可合并为一个 ZIP，也可逐页分别保存。键盘谱/数字谱 TXT 可分别勾选是否保留多声部隐形标记，以及是否包含 `// @jpeditor` 元数据；元数据默认保留，关闭后下次打开无法自动恢复拍号、速度与符号映射，需要手动重新设置。",
  ...
  },
  {
    title: "设置与排版",
    body: [
      "顶部 **设置** 集中外观、文本位置、启动行为、声部颜色和播放音源。外观可选跟随系统、浅色或深色。**拍数位置显示** 可切换底部当前位置的分数/小数表示（7/4 拍或 1.75 拍），不改变谱面拍号或实际时值。**连接跨行小节线** 默认关闭，开启后连接上下谱行对应的小节线。",
      "**设置 → 设置快捷键** 可编辑当前快捷键，支持按键录入、搜索、冲突提示和恢复默认；下方快捷键表显示当前配置。功能说明中的按键为默认值。多声部 TXT 的“文本声部着色”可关闭或恢复文本颜色，保留各声部颜色配置。",
      "工具栏 **乐谱** 修改当前键盘谱/数字谱的识别与节奏参数。**排版** 在悬浮窗口提供纸张、字号、墨色、间距、标题和钢琴系统等参数，同时自动展开最左侧的 **排版样张**；点击 **文本谱** 标签可继续编辑原文。纸张支持 16:9 / 4:3 / A4 / A3；混排隐藏小节号也在排版中。参数实时预览，点击「应用」才保存；关闭或切换有未应用改动的面板时，可应用、放弃或继续编辑。",
      "**乐谱 / 排版窗口** 可拖动标题栏移动，拖右下角调整大小；两个窗口分别记住位置和尺寸。标题栏的复位按钮可恢复默认，窗口始终限制在页面可见区域。窗口外仍可查看和点选谱面。",
      "点击谱面上的标题、副标题或署名可打开 **标题与署名** 悬浮窗口；点击开头的调号、拍号和速度可直接修改这组设置，修改同步到乐谱窗口并支持撤销。五度圈将 C 放在顶部，每个调都标注升降号数量。",
      "折叠或展开设置分类时保持当前滚动位置；靠底部收起后，向上滚动会逐步收回临时空白。花括号默认宽度为 0.35×、粗细为 3.2px，延音线与圆滑线的默认厚度已略微调细。",
      "修改速度、倚音、装饰或谱面文字时，会打开页面内的悬浮输入窗口，可拖动标题栏或调整大小。Enter 确定，Esc 取消；多行文字用 Ctrl/Cmd+Enter 确定。渐快、渐慢的目标速度与终点小节可在同一窗口填写；输入错误会直接在窗口内提示。",
      "设置有未保存的修改时，点击窗口外会出现悬浮提示，可选择 **应用并关闭**、**放弃修改** 或 **继续编辑**。提示关闭前不会改变当前谱面；取消输入、关闭提示或切换文档都不会提交未确认的修改。",
      ...
      "**节奏刻度线** 默认开启。工具栏并列显示 **刻度** 与 **时值**：刻度控制光标移动和刻度线密度，时值控制输入、空格前进与时值调整。两组各有自动和附点按钮，互不替代。再次点击当前档位可回到自动；关闭可见刻度线只隐藏图形，不会禁用方向键节奏编辑。",
    ],
  },
  {
    title: "键盘快捷键",
...
function shortcutTable(): HTMLElement {
  ...
  const rows = SHORTCUT_ACTIONS.map((action) => [
    ...
    bindings[action.id].map(shortcutLabel).join(" · ") || "未设置",
  ]);
...
export function showHelpDialog(app: App): void {
  ...
  const title = el("div", "modal-title", "原琴助手编辑器 · 帮助");
