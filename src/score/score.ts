...
export class ParserTemp {
  ...
  pairTie(): void {
    const ordered = [...this.tieNotes].sort((a, b) =>
      a.absoluteTick.compareTo(b.absoluteTick) || a.pitch - b.pitch || a.chord.voice - b.chord.voice);
    const pending = new Map<string, Note[]>();
    for (const nt of ordered) {
      const key = `${nt.pitch}|${nt.chord.voice}`;
      if (nt.tieEnd) {
      ...
      // A middle note in a tie chain is both a stop and a new start. Process
      // the stop first so A—A—A becomes one continuous performance event.
      if (nt.tieStart) {
        const starts = pending.get(key) ?? [];
        starts.push(nt);
...
export class Chord extends Entry {
  ...
  arpeggio = false;
  /**
   * MIDI pitches covered by the arpeggio. `null` keeps the legacy behavior
   * where every sounding note in the written chord belongs to the roll.
   */
  arpeggioPitches: number[] | null = null;
  dot = 0;
  beams = 0; // 减时线
  beats = 0; // 增时线
  voice = 0;
  ...
  rest = false;
  /**
   * A non-retriggered continuation generated by a duration marker at the
   * beginning of the next slash-score group. It keeps its own rhythm column,
   * but is drawn faintly and tied to the preceding sounding chord.
   */
  transparentContinuation = false;
...
export class Note {
  lyrics: Lyric[] = [];
  /** Session-level score editing flag. Kept in the model so playback/export
   *  can ignore a softly deleted note while the renderer still shows it. */
  softDeleted = false;
  pitch = 0;
  step = " ";
  alter = 0;
...
export class Measure {
  ...
  /** A short opening measure that precedes the first numbered full measure. */
  pickup = false;
  /** Printed measure number; null intentionally suppresses the pickup number. */
  ...
  init(opts?: { keepChords?: boolean; primaryVoice?: boolean }): void {
    this.removeUnused(opts?.keepChords ?? false, opts?.primaryVoice ?? false);
  ...
  private removeUnused(keepChords: boolean, primaryVoice: boolean): void {
    const rem: Chord[] = [];
    const voices = this.entries
      .filter((e): e is Chord => e instanceof Chord)
      .map((ch) => ch.voice);
    const primary = voices.length > 0 ? Math.min(...voices) : 1;
    for (const ent of this.entries) {
      if (!(ent instanceof Chord)) continue;
      const ch = ent;
      if (primaryVoice ? ch.voice !== primary : ch.voice > 1) {
        rem.push(ch);
        continue;
      }
      if (keepChords) continue;
...
export class Part {
  measures: Measure[] = [];
  /** Piano documents use two Parts as independently editable hands. */
  hand: "right" | "left" | null = null;
  /** Full-score grouping label. Parts with the same name form one instrument. */
  instrumentName = "";
  /** Vertical order inside the instrument group; 1 is the top voice. */
  voiceIndex = 1;
  /** Original MIDI track when this part was created by MIDI import. */
  sourceTrack: number | null = null;
...
export type TempoMarkKind = "accel" | "rit" | "tempo";
export type TempoBeatUnit = "quarter" | "dotted-quarter" | "eighth";

/** Convert the internal quarter-note BPM to the number printed for a beat unit. */
export function tempoBpmForUnit(quarterBpm: number, unit: TempoBeatUnit): number {
  if (unit === "eighth") return quarterBpm * 2;
  if (unit === "dotted-quarter") return quarterBpm / 1.5;
  return quarterBpm;
}

/** Convert a displayed metronome value back to internal quarter-note BPM. */
export function quarterBpmFromUnit(displayBpm: number, unit: TempoBeatUnit): number {
  if (unit === "eighth") return displayBpm / 2;
  if (unit === "dotted-quarter") return displayBpm * 1.5;
  return displayBpm;
}

export function formatTempoBpm(bpm: number): string {
  const rounded = Math.round(Math.max(0.1, bpm) * 10) / 10;
  return Number.isInteger(rounded) ? String(rounded) : rounded.toFixed(1);
}

/** A score-level tempo annotation anchored to one notated measure position. */
export class TempoMark {
  measure = 0;
  offset = new Fraction(0);
  kind: TempoMarkKind = "tempo";
  bpm: number | null = null;
  beatUnit: TempoBeatUnit = "quarter";
  /** Soft deletion keeps the mark visible as a translucent restore target. */
  softDeleted = false;
}
...
export class Score {
  ...
  ensemble = false;
  /** Playback/export tempo. Old files without Tempo keep the historical 90 BPM default. */
  tempoBpm = 90;
  /** Printed beat unit; playback/export remain normalized to quarter-note BPM. */
  ...
  subtitle = "";
  arranger = "";
  /** Displayed before the first paired piano system. */
  instrumentName = "";
...
/**
 * Normalize an opening anacrusis without stretching its real duration.
 *
 * Some MIDI files encode the pickup itself as a temporary short meter
 * (for example 6/16 followed by 4/4). Other import paths keep the full meter
 * but simply produce a short first measure. Both forms represent the same
 * notation: the header uses the governing full meter, the pickup is
 * unnumbered, and the following full measure is measure 1 without a repeated
 * inline time signature.
 */
export function normalizeOpeningPickup(score: Score): boolean {
  const firstMeasures = score.parts
    .map((part) => part.measures[0])
    .filter((measure): measure is Measure => measure !== undefined);
  if (firstMeasures.length === 0) return false;

  const first = firstMeasures[0];
  const second = score.parts
    .map((part) => part.measures[1])
    .find((measure): measure is Measure => measure !== undefined) ?? null;
  const firstMeterDuration = first.time.beats * 4 / first.time.beatType;
  const firstActualDuration = Math.max(...firstMeasures.map((measure) => measure.duration.toFloat()));
  ...
  const encodedAsShortMeter = second !== null &&
    ...
    firstActualDuration <= firstMeterDuration + tolerance;
  if (!explicitlyMarked && !shortInFullMeter && !encodedAsShortMeter) return false;

  const governingTime = encodedAsShortMeter || (
    explicitlyMarked &&
    second !== null &&
    !sameTime(first.time, second.time) &&
    firstActualDuration < second.time.beats * 4 / second.time.beatType - tolerance
  )
    ? second!.time
    : first.time;

  for (const part of score.parts) {
    const opening = part.measures[0];
    if (!opening) continue;
    opening.pickup = true;
    opening.displayNumber = null;
    opening.time = new Time(governingTime.beats, governingTime.beatType);
    opening.timeChange = false;

    let number = 1;
    for (let index = 1; index < part.measures.length; index++) {
      const measure = part.measures[index];
      measure.displayNumber = number++;
      if (index === 1 && sameTime(measure.time, governingTime)) {
...
/** Meter printed in the publication header, independent of pickup length. */
export function scoreOpeningTime(score: Score): Time {
  const measures = score.parts[0]?.measures;
  if (!measures || measures.length === 0) return new Time();
  if (!measures[0].pickup) return measures[0].time;
  return measures.find((measure, index) => index > 0 && !measure.pickup)?.time ?? measures[0].time;
}
