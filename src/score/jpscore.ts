...
class JpScore {
  ...
  private chordVoice(ch: Chord): string {
    // The Score model keeps chord tones from top to bottom so notes[0]
    // remains the melodic/top note used by layout, lyrics and tie geometry.
    // JPW bracket syntax, however, is conventionally written bottom to top
    // (`[135]`). Sort only the serialized view instead of mutating the model:
    // mutating it makes a continuation chord change vertical geometry when a
    // duration is respelled across a beat or barline.
    const pitches = [...ch.notes]
      .sort((left, right) => left.pitch - right.pitch)
      .map((nt) => this.pitchVoice(nt));
