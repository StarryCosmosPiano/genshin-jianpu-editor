...
#   scripts/release.sh 0.1.0 "修复 X" "新增 Y"
set -euo pipefail

VER="${1:?用法: scripts/release.sh <version> [notes...]}"
shift || true
TAG="v${VER}"
REPO="StarryCosmosPiano/genshin-jianpu-editor"
cd "$(dirname "$0")/.."

# 约定：本项目提交/推送用 StarryCosmosPiano 帐号
gh auth switch --user StarryCosmosPiano >/dev/null 2>&1 || true
